...
pub fn make_3d_lut_cube(
    ...
    out.push_str("DOMAIN_MIN 0.0 0.0 0.0\nDOMAIN_MAX 1.0 1.0 1.0\n");
    let decode = |v:f64| if shaper_size > 0 { v } else { tf_decode(v, src_tf) };
    for b in 0..size { for g in 0..size { for r in 0..size {
        ...
        let v = Vector3::new(decode(rf), decode(gf), decode(bf));
        let v_lin_dst: Vector3<f64> = m * v; // gamut conversion in linear
        let rd = tf_encode(v_lin_dst[0], dst_tf).clamp(0.0, 1.0);
        let gd = tf_encode(v_lin_dst[1], dst_tf).clamp(0.0, 1.0);
        let bd = tf_encode(v_lin_dst[2], dst_tf).clamp(0.0, 1.0);
        out.push_str(&format!("{:.10} {:.10} {:.10}\n", rd, gd, bd));
    }}}
    out
}

...
pub enum MetadataValue {
    ...
    Opaque { kind: String, data: Vec<u8> },
}

#[cfg(feature = "use_exr_crate")]
pub fn read_metadata(path: &Path) -> Result<BTreeMap<String, MetadataValue>> {
    use exr::meta::{attribute::AttributeValue, MetaData};

    let meta = MetaData::read_from_file(path, false)?;
    let header = meta.headers.get(0).ok_or_else(|| anyhow!("no header"))?;

    let mut out = BTreeMap::new();

    // time code stored in shared attributes
    if let Some(tc) = header.shared_attributes.time_code {
        if let Some(t) =
            NaiveTime::from_hms_opt(tc.hours as u32, tc.minutes as u32, tc.seconds as u32)
        {
            let date = NaiveDate::from_ymd_opt(1970, 1, 1).unwrap();
            out.insert(
                "timeCode".to_string(),
                MetadataValue::DateTime(NaiveDateTime::new(date, t)),
            );
        }
    }

    // capture date stored as text; try to parse
    if let Some(cd) = &header.own_attributes.capture_date {
        let s = String::from_utf8_lossy(cd.as_slice()).to_string();
        if let Ok(dt) = NaiveDateTime::parse_from_str(&s, "%Y:%m:%d %H:%M:%S") {
            out.insert("captureDate".to_string(), MetadataValue::DateTime(dt));
        } else {
            out.insert("captureDate".to_string(), MetadataValue::String(s));
        }
    }
    ...
    for (name, value) in iter {
        let key = String::from_utf8_lossy(name.as_slice()).to_string();
        let mv = match value {
            AttributeValue::I32(v) => MetadataValue::Int(*v),
            AttributeValue::F32(v) => MetadataValue::Float(*v as f64),
            AttributeValue::F64(v) => MetadataValue::Float(*v),
            AttributeValue::Text(t) => {
                MetadataValue::String(String::from_utf8_lossy(t.as_slice()).to_string())
            }
            AttributeValue::TimeCode(tc) => {
                    ...
                    NaiveTime::from_hms_opt(tc.hours as u32, tc.minutes as u32, tc.seconds as u32)
                {
                    ...
                    MetadataValue::Opaque {
                        kind: "TimeCode".to_string(),
                        data: Vec::new(),
                    }
                }
            }
            AttributeValue::Custom { kind, bytes } => MetadataValue::Opaque {
                kind: String::from_utf8_lossy(kind.as_slice()).to_string(),
                data: bytes.clone(),
            },
            other => {
                let mut data = Vec::new();
                other.write(&mut data)?;
                let kind = String::from_utf8_lossy(other.kind_name()).to_string();
                MetadataValue::Opaque { kind, data }
            }
        };
        out.insert(key, mv);
    }
...
pub fn read_metadata(_path: &Path) -> Result<BTreeMap<String, MetadataValue>> {
    Err(anyhow!("feature 'use_exr_crate' is required"))
}

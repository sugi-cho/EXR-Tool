...
use exrtool_core::rules::{apply_rules, FileChange, ReplacementRule};
use exrtool_core::{
    export_png, generate_preview, load_exr_basic, make_1d_lut, parse_cube, ColorSpace,
};
use std::fs;
use std::path::PathBuf;

fn write_log(path: PathBuf, entries: &[FileChange]) -> Result<()> {
    match path.extension().and_then(|s| s.to_str()) {
        ...
        _ => return Err(anyhow::anyhow!("log path must end with .json or .csv")),
    }
    Ok(())
}
...
fn main() -> Result<()> {
    ...
    match cli.command {
        ...
        Commands::Apply {
            ...
            let rule = ReplacementRule::new(&find, &replace);
            let logs = apply_rules(&inputs, &[rule])?;
            if let Some(p) = log {
                write_log(p, &logs)?;
            }
            for l in logs {

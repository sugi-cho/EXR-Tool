...
use exrtool_core::{
    export_png, generate_preview, load_exr_basic, make_1d_lut, parse_cube, ColorSpace, ToneMapKind,
    ToneMapOrder,
};
use std::fs;
...
enum Commands {
    ...
    Preview {
        ...
        #[arg(long)]
        lut: Option<PathBuf>,
        /// トーンマッピング: none | aces | filmic
        #[arg(long, default_value = "none")]
        tonemap: String,
        /// トーンマッピングの適用順序: pre | post
        #[arg(long, default_value = "post")]
        tonemap_order: String,
    },
...
fn main() -> Result<()> {
    ...
    match cli.command {
        Commands::Preview {
            ...
            let parse_tm = |s: &str| -> Result<ToneMapKind> {
                match s.to_ascii_lowercase().as_str() {
                    "none" => Ok(ToneMapKind::None),
                    "aces" => Ok(ToneMapKind::Aces),
                    "filmic" => Ok(ToneMapKind::Filmic),
                    _ => Err(anyhow::anyhow!("unknown tonemap: {}", s)),
                }
            };
            let parse_order = |s: &str| -> Result<ToneMapOrder> {
                match s.to_ascii_lowercase().as_str() {
                    "pre" | "before" => Ok(ToneMapOrder::BeforeLut),
                    "post" | "after" => Ok(ToneMapOrder::AfterLut),
                    _ => Err(anyhow::anyhow!("unknown tonemap order: {}", s)),
                }
            };
            let tm = parse_tm(&tonemap)?;
            let order = parse_order(&tonemap_order)?;
            let preview =
                generate_preview(&img, max_size, exposure, gamma, lut_obj.as_ref(), tm, order);
